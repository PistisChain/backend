...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        this.app.get('/api/attendance/student', (req, res) => {
            const { studentId, startDate, endDate, year, weekNumber, monthNumber, granularity } = req.query;
  ...
  : null;
                ...
                // 按粒度处理时间范围
                if (granularity === 'week' && year && weekNumber) {
                    const startOfWeek = getFirstDayOfWeek(year, weekNumber); // 目标周的周一
                    startDateTime = new Date(startOfWeek.setHours(0, 0, 0, 0)); // 周一的开始时间
                    endDateTime = new Date(startOfWeek);
                    endDateTime.setDate(startOfWeek.getDate() + 6); // 周日的结束时间
                    endDateTime.setHours(23, 59, 59, 999);
                } else if (granularity === 'month' && year && monthNumber) {
                    startDateTime = new Date(year, monthNumber - 1, 1, 0, 0, 0); // 月初
                    endDateTime = new Date(year, monthNumber, 0, 23, 59, 59); // 月末
                }
...
this.app.get('/api/attendance/course', (req, res) => {
    const { eventId, startDate, endDate, year, weekNumber, monthNumber, granularity } = req.query;

    try {
        // 如果未提供 eventId，则返回空数组
        if (!eventId) {
            res.send([]);
            return;
        }
        ...
        const filteredData = attendanceRecords.filter(record => {
            const recordDate = new Date(record.timestamp * 1000); // 转换记录的时间戳
            const eventValid = record.eventId === eventId; // 必须匹配 eventId
            const startDateValid = startDateTime ? recordDate >= startDateTime : true; // 开始日期校验
            const endDateValid = endDateTime ? recordDate <= endDateTime : true; // 结束日期校验
            return eventValid && startDateValid && endDateValid;
        });
        ...
        res.status(500).send({ error: 'Failed to fetch class attendance data' });
    }
});
