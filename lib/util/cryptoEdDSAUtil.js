...
class CryptoEdDSAUtil {
    static generateSecret(password) {
        const timestamp = Date.now(); // 或者 Math.floor(Date.now() / 1000) 获取秒级时间戳

        // 将时间戳与密码结合
        const combinedInput = `${password}:${timestamp}`;

        let secret = crypto.pbkdf2Sync(combinedInput, SALT, 10000, 512, 'sha512').toString('hex');
