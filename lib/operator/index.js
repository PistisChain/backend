...
const OPERATOR_FILE = 'user.json';

class Operator {
    constructor(dbName, blockchain, id, role) {
    ...
    getWalletById(walletId) {
        return R.find((wallet) => {console.log(wallet.id);
            return wallet.id == walletId; }, this.wallets);
    }
    ...
    createRegister(walletId, address, amount, studentId, eventId, timestamp, passwordHash) {
        let utxo = this.blockchain.getUnspentTransactionsForAddress(address);
        let wallet = this.getWalletById(walletId);
        if (wallet == null) throw new ArgumentError(`Wallet not found with id '${walletId}'`);       
        let secretKey = wallet.getSecretKeyByAddress(address);        
        if (secretKey == null) throw new ArgumentError(`Secret key not found with Wallet id '${walletId}' and address '${address}'`);
        let tx = new TransactionBuilder();
        tx.from(utxo);
        ...
        tx.passwordHash = passwordHash;
        return Transaction.fromJson(tx.build());
    }

    createAttendance(walletId, address, studentId, eventId, timestamp) {
        let wallet = this.getWalletById(walletId);
        let signInHash = CryptoUtil.hash({
            studentId: studentId,
            eventId: eventId,
            timestamp: timestamp
        });
        ...
        if (wallet == null) throw new ArgumentError(`Wallet not found with id '${walletId}'`);       
        let secretKey = wallet.getSecretKeyByAddress(address);
        const keyPairs =  CryptoEdDSAUtil.generateKeyPairFromSecret(secretKey);
        const signature =  CryptoEdDSAUtil.signHash(keyPairs, signInHash);
        if (secretKey == null) throw new ArgumentError('Secret key not found');
        let tx = new TransactionBuilder();
        tx.from(utxo);
        tx.to(address, 0);
        tx.sign(secretKey);        
        ...
        tx.eventId = eventId;
        tx.timestamp = timestamp;
        tx.attendanceSignature = signature;
        return Transaction.fromJson(tx.build());
    }
}
